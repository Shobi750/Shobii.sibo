...
try:
    key1 = open('/data/data/com.termux/files/usr/bin/.mrSHOBIxxx-cov', 'r').read()
except:
    kok=open('/data/data/com.termux/files/usr/bin/.mrSHOBIxxx-cov', 'w');kok.write(myid);kok.close()
uid = os.getuid()
key1 = open('/data/data/com.termux/files/usr/bin/.mrSHOBIxxx-cov', 'r').read()
kex=(f"SHOBI-XD~CREATE:{uid}TS{key1}110E==")
...
------------------------------------------
 Author : Mr. SHOBI
 Github : SHOBI750(Follow Must)
 Tools  :  TO BHOSRI K HAM HUY PRINCE 
 Contact: +923129628965
 Version: 1.6
\033[1;37m------------------------------------------""") 
...
def Create():
    ...
    def strt():
       try:
           ...
           import sys
           loop+=1
           sys.stdout.write('\r\r\033[1;37m [SHOBI-CREATE] \033[1;32mOK:%s \033[1;37m'%(ok));sys.stdout.flush()
           requests=r.Session()
           from fake_email import Email
           mmail=Email().Mail()
           em=mmail['mail']
           ...
           if "9":
              ...
              if requests=="chk":
                ...
                pass
                 ...
                 print("\r\r\033[1;32m [SHOBI-OK] "+uid+'|'+pas+'|'+coki)
                 ok+=1
           ...
           pass
    file="/sdcard/SHOBI/CREATE-OK.txt"
    u=5000
    clear()
    print(' Use Airplane Or Use IP Changer if ids coming cp ')
    print(' ids will be save in /sdcard/SHOBI/CREATE-OK.txt')
    linex()
